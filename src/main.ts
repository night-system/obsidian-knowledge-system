...
import { REVIEW_VIEW_TYPE, ensureReviewBase, registerReviewBasesView, unregisterReviewBasesView, regenerateReviewBaseFile } from './reviewView';
import { TIDY_VIEW_TYPE, ensureTidyBase, registerTidyBasesView, unregisterTidyBasesView, regenerateTidyBaseFile } from './tidyView';
import { PANEL_VIEW_TYPE, ensurePanelBase, registerPanelBasesView, unregisterPanelBasesView, regeneratePanelBaseFile } from './panelView';
...
export default class KnowledgeSystemPlugin extends Plugin {
   ...
   */
  private leafForOpen(targetViewType: string, targetPath?: string): WorkspaceLeaf | null {
    if (this.settings.panelOpenMode !== 'current') {
      return this.app.workspace.getLeaf('tab');
    }
    const leaf = this.app.workspace.getLeaf(false)
      ?? this.app.workspace.getLeavesOfType('markdown')[0]   // 当前正显示笔记的 leaf，直接替换
      ?? this.app.workspace.getLeaf('tab');
    if (!leaf) return null;
    if (leaf.view) {
      const fileView = leaf.view as { file?: { path?: string } | null };
      const sameTarget = leaf.view.getViewType() === targetViewType
        && (!targetPath || fileView.file?.path === targetPath);
      if (sameTarget) {
  ...
  async activateView(): Promise<void> {
    ...
    if (!leaf) return;   // 已是设置视图 → 已 reveal
  ...
  async openReviewView(): Promise<void> {
    ...
    if (!leaf) return;   // 已是该审核面板 → 已 reveal
  ...
  async openTidyView(): Promise<void> {
    ...
    if (!leaf) return;   // 已是该整理面板 → 已 reveal
    await leaf.openFile(baseFile);
    this.app.workspace.revealLeaf(leaf);
  }
